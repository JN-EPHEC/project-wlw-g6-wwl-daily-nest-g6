...
export default function ShoppingList() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, snapshot => {
      const fams: any[] = [];
      snapshot.forEach(doc => fams.push({ id: doc.id, ...doc.data() }));
      setFamilies(fams);
    });
    return unsubscribe;
  }, [user]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Picker
        selectedValue={selectedListType === "personal" ? "personal" : selectedFamily?.id}
        onValueChange={(value) => {
          if (value === "personal") {
      ...
      >
        <Picker.Item label="Liste personnelle" value="personal" />
        {families.map(f => <Picker.Item key={f.id} label={f.name} value={f.id} />)}
      </Picker>
      </View>
